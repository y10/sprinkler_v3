...
This processes HTML/CSS/JS files from `html/` directory:
1. Copies files to `arduino/html/`
2. Bundles and minifies JavaScript (index.js, setup.js)
3. Inlines CSS and base64 encodes images
4. Gzips all assets
5. Converts to C header files (.h) with byte arrays
6. Generates version headers from `.sprinkler/settings.json`
...
- **Sketch folder**: `arduino/` (contains `arduino.ino`)
...
- Various gulp plugins for minification and asset processing
...
- Never edit files in `arduino/html/` directly - they are generated by gulp
- The build process must run `gulp` before `arduino-cli compile` to generate headers
